...
def _formater_liste(elements: list[str], texte_si_vide: str) -> str:
    if not elements:
...
def _construire_message_utilisateur(
    ...
    etablissements = _formater_liste(
        [
    ...
    historique_financier = _formater_liste(
        [
    ...
    procedures_collectives = _formater_liste(
        contexte.procedures_collectives, "aucune procédure collective détectée"
    )
    changements_recents = _formater_liste(
